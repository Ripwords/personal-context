...
test("resolveWritebackItems includes events and scheduled todos, skips unscheduled todos", async () => {
  const ev = await createEvent(db, {
    title: "Standup",
    startsAt: new Date("2026-07-01T09:00:00Z"),
    endsAt: new Date("2026-07-01T09:15:00Z"),
  });
  const scheduled = await createTodo(db, {
    title: "Check project",
  ...
  const items = await resolveWritebackItems(db, [
    { kind: "event", id: ev.id },
    { kind: "todo", id: scheduled.id },
    { kind: "todo", id: anytime.id },
  ]);

  expect(items.map((i) => i.title).sort()).toEqual(["Check project", "Standup"]);
